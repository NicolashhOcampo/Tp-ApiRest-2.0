...
public interface PersonaRepository extends BaseRepository<Persona, Long> {
    ...
    @Query(value = "SELECT p FROM Persona p WHERE p.nombre LIKE %:filtro% OR p.apellido LIKE %:filtro%") //"?1" hace referencia al primer parametro
    ...
    @Query(
            value = "SELECT * FROM Persona p WHERE p.nombre LIKE %:filtro% OR p.apellido LIKE %:filtro%",
            countQuery = "SELECT count(*) FROM persona p",
            nativeQuery = true
    )
